...
transform = transforms.Compose([
    transforms.Resize((64, 64)),  # Resize images to 1024x1024
    transforms.ToTensor()
])

dataset = TextImageDataset('data/dataset.csv', 'data/images', transform=transform)
dataloader = DataLoader(dataset, batch_size=32, shuffle=True)  # Set batch size to 4
...
for epoch in range(40):  # Set number of epochs to 300
    ...
    print(f"Epoch [{epoch+1}/40], Loss: {epoch_loss/len(dataloader):.3f}, LR: {scheduler.get_last_lr()[0]:.6f}")
    continue
