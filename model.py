...
class TextToImageModel(nn.Module):
    def __init__(self, output_size=(256, 256)):  # Allow dynamic output size
        ...
        # Output size changes dynamically
        self.fc3 = nn.Linear(1024, self.output_size[0] * self.output_size[1] * 3)  # Output for image generation
    ...
    def forward(self, text_input):
        ...
        # Dynamically reshape based on the requested output size
        x = x.view(-1, 3, self.output_size[0], self.output_size[1])  # Reshape to image size (batch_size, 3, H, W)
        return x
